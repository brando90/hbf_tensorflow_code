...
for k in number_units_list:
    ...
    # print
    print 'tensorflow error: ', sklearn.metrics.mean_squared_error(X_reconstruct_tf, X_train)
    print 'keras error: ',(1.0/N_train_keras)*LA.norm(X_reconstruct_keras - x_train)**2
    print 'U error: ',(1.0/X_train.shape[0])*LA.norm(X_reconstruct_tf - X_train)**2
    print 'RBF error: ',(1.0/X_train.shape[0])*LA.norm(np.squeeze(Y_pred_train_best) - X_train)**2
    #pdb.set_trace()
