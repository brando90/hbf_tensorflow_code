...
def bt_mdl_conv(arg,x):
    ...
    print('l ', l)
    print('arg.F', arg.F)
    print('nb_filters ', None)
    print(conv)
    for l in range(0,len(arg.F)):
        print('--')
        print('l ', l)
        print('arg.F', arg.F)
        print('nb_filters ', arg.F[l])
        conv = get_activated_conv_layer(l,arg,x=conv,kernel_size=[1,kernel_width],stride=[1,kernel_width],scope=arg.scope_name+str(l))
        print(conv)
